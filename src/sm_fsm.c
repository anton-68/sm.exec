        ...
        switch (state) {
            ...
            case SM_S:  
                ...
                if((fsm->name = tostr(fsm_json, t)) == NULL) {
                    SM_SYSLOG(SM_CORE, SM_LOG_ERR, "Error parsing FSM name", "Malformed FSM JSON");
                    return cleanup(fsm, tokens);
                }
...
//                    printf("\nJSMN token : %s", tostr(fsm_json, t));
            ...
            case N_NAME:
                if(t->type == JSMN_STRING) {
                    ...
                    SM_SYSLOG(SM_CORE, SM_LOG_ERR, "FSM node name must be string", "Malformed FSM JSON");
                    return cleanup(fsm, tokens);
                }                                      
                state = NODE_O;
                break;
            ...
            case N_ID:
                if((node_id = toint(fsm_json, t)) < 0) {
                    SM_SYSLOG(SM_CORE, SM_LOG_ERR, "Error parsing node id", "Malformed FSM JSON");
                    return cleanup(fsm, tokens);
                }   
                state = NODE_O;
                break;
...
//                    printf("\nJSMN token : %s", tostr(fsm_json, t));
            ...
            case T_TYPE:
                if(t->type == JSMN_STRING){
                    if(streq(fsm_json, t, "regular")) {
                        ...
                        SM_SYSLOG(SM_CORE, SM_LOG_ERR, "Unknown FSM transition type", "Malformed FSM JSON");
                        return cleanup(fsm, tokens);
                    }
                }
                else {
                    SM_SYSLOG(SM_CORE, SM_LOG_ERR, "FSM transition type must be string", "Malformed FSM JSON");
                    return cleanup(fsm, tokens);
                }
                state = TRANSITION_O;
                break;
            ...
            case T_REF:
                if((transition_ref_name = tostr(fsm_json, t)) == NULL) {
                    SM_SYSLOG(SM_CORE, SM_LOG_ERR, "Error parsing transition reference name", "Malformed FSM JSON");
                    return cleanup(fsm, tokens);
                }
                ...
                if(node_transitions[transition_idx].transitionRef == NULL){
                    sprintf(er, "Transition app %s not found in the directory", transition_ref_name);
                    SM_SYSLOG(SM_CORE, SM_LOG_ERR, er, "Malformed FSM JSON");
            ...
            case T_SET: 
                if((node_transitions[transition_idx].setEventId = toint(fsm_json, t)) < 0) {
                    SM_SYSLOG(SM_CORE, SM_LOG_ERR, "Error parsing set event id", "Malformed FSM JSON");
                    return cleanup(fsm, tokens);
                }
                state = TRANSITION_O;
                break;
            ...
            case EVENTS_A:
                if(t->type == JSMN_ARRAY) {
                    ...
                    if((fsm->events = calloc(noe_c, sizeof(sm_fsm_event))) == NULL) {
                        SM_SYSLOG(SM_CORE, SM_LOG_ERR, "Error allocating memory for FSM event descriptors", "Malformed FSM JSON");
                        return cleanup(fsm, tokens);
                    }
                    ...
                    SM_SYSLOG(SM_CORE, SM_LOG_ERR, "FSM event descriptors must be stored in array", "Malformed FSM JSON");
                    return cleanup(fsm, tokens);
                }
                
            case EVENT_OBJ:
                if(t->type != JSMN_OBJECT) {
                    SM_SYSLOG(SM_CORE, SM_LOG_ERR, "FSM JSON event root element must be object", "Malformed FSM JSON");
                    return cleanup(fsm, tokens);
                }
                e_c = t->size;
                if(e_c < 2){
                    SM_SYSLOG(SM_CORE, SM_LOG_ERR, "FSM JSON event header incomplete", "Malformed FSM JSON");
                    return cleanup(fsm, tokens);
                }
                state = EVENT_O;
                break;    
            ...
            case E_ID: 
                if((fsm->events[event_idx].id = toint(fsm_json, t)) < 0) {
                    SM_SYSLOG(SM_CORE, SM_LOG_ERR, "Error parsing event id", "Malformed FSM JSON");
                    return cleanup(fsm, tokens);
                }
                state = EVENT_O;
                break;
            ...
            case E_NAME:
                if(t->type == JSMN_STRING) {
                    ...
                    if((fsm->events[event_idx].name = tostr(fsm_json, t)) == NULL) {
                        SM_SYSLOG(SM_CORE, SM_LOG_ERR, "Error parsing event name", "Malformed FSM JSON");
                        return cleanup(fsm, tokens);
                    }
                }
                else {
                    SM_SYSLOG(SM_CORE, SM_LOG_ERR, "FSM event name must be string", "Malformed FSM JSON");
                    return cleanup(fsm, tokens);
                }                                      
                state = EVENT_O;
                break;
    ...
    if(state != STOP && i < num_of_tokens) {
        SM_SYSLOG(SM_CORE, SM_LOG_ERR, "FSM JSON exit state != STOP", "Malformed FSM JSON");
        return cleanup(fsm, tokens);
    }
...
char *sm_fsm_to_string(sm_fsm* f, sm_directory *dir) {  
    ...
    sprintf(line, "The final node: %ld\n", f->final);
    strcat(sm_buffer, line);
    sprintf(line, "The initial node: %ld\n", f->initial);
    strcat(sm_buffer, line);
