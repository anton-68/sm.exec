...
-- Load apps 
local lib = sm.loadlib(g.getdir("app").."Scripts/Lua/TrafficSM/sm_robot_demo_apps.so")
...
do_step1 = sm.new_event(SM_TRSIM_STEP1, 128)
do_step2 = sm.new_event(SM_TRSIM_STEP2, 128)
do_step3 = sm.new_event(SM_TRSIM_STEP3, 128)
do_plan = sm.new_event(SM_TRSIM_PLAN, 128)
...
for i,b in ipairs(bots) do
    e = sm.new_event(SM_TRSIM_BCME, 128)
...
bf_pool = sm.new_queue(w * h // 2, 128, false)    
